...
- Use `inventory.toml` to select target hosts in the `etcd` group
...
- `etcd.peer_host` and `etcd.client_host` in `inventory.toml` must be reachable addresses between nodes

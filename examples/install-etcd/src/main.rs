...
fn hydrate_etcd_host_defaults(inventory: &mut Inventory) -> anyhow::Result<()> {
    for host in inventory.hosts_mut() {
        let explicit_peer_host = host.remote().get_var_optional("etcd.peer_host")?;
        let facts_hostname = host.remote().get_var_optional("rusible.facts.hostname")?;
        let client_host = host.remote().get_var_optional("etcd.client_host")?;
        let peer_host = explicit_peer_host.clone().or(facts_hostname);
        let peer_host = peer_host.with_context(|| {
            format!(
